...
class TextWriter:
    ...
    def write_table(self, table_info, lang, lines):
        if lang == "mr":
            ...
            lines.extend(
                r["mr"].replace("|", " | ").strip() for r in table_info["rows"]
            )
            lines.append("-" * total_width)
            return
        ...
        col_widths = [
            max((len(r[idx]) if idx < len(r) else 0) for r in rows)
    ...
    def __call__(self, doc):
        ...
        for page_idx, page in enumerate(doc.pages):
            ...
            para_table_idx_dict = {}
            for (para_idx, table_idxs) in groupby(
                page.table_para_idxs, key=itemgetter(1)
            ):
            ...
            for para_idx, para in enumerate(page.paras + [None]):
                if para_idx in para_table_idx_dict:
                    for table_idx in para_table_idx_dict[para_idx]:
                        ...
                        table_info = self.build_table_info(
                            page.tables[t], page.table_trans[t]
                        )
                        for (lang, lines) in lang_lines_dict.items():
